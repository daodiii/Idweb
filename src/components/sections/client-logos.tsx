  ...
  return (
    <section className="relative overflow-hidden border-y border-[var(--color-dark-border)] bg-[var(--color-dark-bg)] py-10">
      <div className="mx-auto mb-6 max-w-6xl px-6 text-center">
        <p className="text-xs font-semibold uppercase tracking-[3px] text-[var(--color-dark-muted)]">
        ...
        {[...Array(4)].map((_, setIndex) => (
          ...
          >
            {CLIENT_NAMES.map((name) => (
              <span
                key={name}
                className="whitespace-nowrap font-serif text-lg font-black tracking-tight text-[var(--color-dark-muted)] opacity-40 transition-opacity duration-300 hover:opacity-80 sm:text-xl"
              >
                {name}
              </span>
            ))}
